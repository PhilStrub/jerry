...
import type { Message as TMessage, UseChatHelpers } from "ai/react";
    ...
    <div className="flex flex-col mb-2 group">
      {isReasoning ? (
        <div className={cn(
          ...
          "bg-primary/5 text-primary border border-primary/20 w-fit"
        )}>
        ...
        <div className="flex flex-col w-full space-y-3">
          {(() => {
            const isLiveAssistantMessage =
              isLatestMessage && status !== "ready" && message.role === "assistant";
              ...
              case "tool-invocation":
                ...
                return (
                  <ToolInvocation
                    ...
                    append={append}
                    isFromDatabase={!isLiveAssistantMessage}
                  />
                );
              case "reasoning":
                return (
          ...
          {/* Note: htmlResources for database messages are handled by the artifact system */}
          {/* The chat component auto-loads the latest artifact on chat load */}
          {/* Tool invocations handle their own resource display logic */}
...
// Memoized export so other components (e.g., Messages.tsx) can import { Message }
