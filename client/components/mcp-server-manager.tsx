...
import { DeleteServerDialog } from "./ui/delete-server-dialog";
...
interface MCPServerManagerProps {
    ...
    embedded?: boolean; // when true, render inline without modal
}
...
export const MCPServerManager = ({
    ...
    const content = (
                        ...
                        {servers.length > 0 ? (
                            <div className="flex-1 overflow-hidden flex flex-col">
                                <div className="flex-1 overflow-hidden flex flex-col">
                                    ...
                                    <div className="overflow-y-auto pr-1 flex-1 gap-4 flex flex-col pb-16">
                                        {servers
                                            .sort((a, b) => {
                                            ...
                                            return (
                                                <div
                                                    key={server.id}
                                                    className={`
                            relative flex flex-col p-4 rounded-xl transition-colors
                            border ${isActive
                          ...
                          `}
                                                >
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        className={cn(
                                                            "w-full gap-1.5 rounded-lg",
                                                            isActive && "bg-muted-foreground/90 text-white hover:bg-muted-foreground hover:text-white dark:bg-muted-foreground/80 dark:hover:bg-muted-foreground"
                                                        )}
                            ...
                            <div className="grid gap-1.5">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid gap-2 grid-cols-2">
                                        <button
                                            ...
                                            className={`flex items-center gap-2 p-3 rounded-md text-left border transition-all ${
                                                newServer.type === 'streamable-http' 
                                                    ? 'border-muted-foreground/30 bg-muted/50 ring-1 ring-muted-foreground/30' 
                                                    : 'border-border hover:border-border/80 hover:bg-muted/50'
                                            }`}
                                        >
                                            <Globe className={`h-5 w-5 shrink-0 ${newServer.type === 'streamable-http' ? 'text-muted-foreground' : ''}`} />
                                            <div>
                                                <p className="font-medium">Streamable HTTP</p>
                                                <p className="text-xs text-muted-foreground">Modern HTTP with streaming</p>
                                            </div>
                                        </button>
                                        ...
                                        >
                                            <Terminal className={`h-5 w-5 shrink-0 ${newServer.type === 'stdio' ? 'text-muted-foreground' : ''}`} />
                                            <div>
                            ...
                            {newServer.type === 'streamable-http' ? (
                                <div className="grid gap-1.5">
                                    ...
                                    <Input
                                        ...
                                        placeholder="https://mcp.example.com/token"
                                        className="relative z-0"
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        Full URL to the Streamable HTTP endpoint of the MCP server
                                    </p>
                                </div>
                            ) : (
                                <>
                                    ...
                                    <AccordionContent>
                                        <div className="space-y-3">
                                            ...
                                            {newServer.headers && newServer.headers.length > 0 ? (
                                                ...
                                                <p className="text-xs text-muted-foreground text-center py-2">
                                                    No {newServer.type === 'streamable-http' ? 'headers' : 'additional configuration'} added
                                                </p>
                                            )}
                                            <p className="text-xs text-muted-foreground">
                                                {newServer.type === 'streamable-http'
                                                    ? 'HTTP headers will be sent with requests to the Streamable HTTP endpoint.'
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 flex justify-between z-10">
                    {view === 'list' ? (
                        <>
                            <Button
                                variant="outline"
    ...
    if (embedded) {
        return (
            <div className="relative max-h-[70vh] overflow-hidden flex flex-col">
