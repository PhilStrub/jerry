...
export function MCPProvider({ children }: { children: React.ReactNode }) {
  ...
  const startServer = async (serverId: string): Promise<boolean> => {
    ...
    try {
      // For Streamable HTTP servers, just check if the endpoint is available
  ...
  // Calculate mcpServersForApi based on current state - memoized to prevent unnecessary re-renders
  const mcpServersForApi = useMemo(() => {
    return getActiveServersForApi();
